...
: SEE
	DICT DUP 0= IF ." Word not found" CR EXIT THEN HERE @ LATEST @
...
VARIABLE HISTORY_PTR NIL HISTORY_PTR !
: ALLOCATE_HISTORY_LINE HISTORY_PTR @ 256 ALLOT CONS HISTORY_PTR ! ;
...
: ACCEPT CLEAR_TIB BEGIN ECHO DUP NORMAL_CHAR OVER = UNTIL DROP ;
: READ_LINE NL ACCEPT ;
...
: GPIO_ENABLE_REG ( gpio -- sel a b ) 10 /MOD 4 * GPFSEL1 + SWAP 3 * DUP 3 + ;
: GPIO_ENABLE ROT DUP @ 2SWAP SWAP ROT BITRANGE SWAP NTHBIT

: LEDENABLE GPFSEL4 @ 21 24 CLEARBITS 21 SETBIT GPFSEL4 ! ;
: LEDON 47 GPIO_ON ;
: LEDOFF 47 GPIO_OFF ;
...
32 CELLS ALLOT CONSTANT LOOPSTACK
VARIABLE LOOPSP 
LOOPSTACK LOOPSP !
: >LOOP LOOPSP @ ! 4 LOOPSP +! ;
: LOOP> 4 LOOPSP -! LOOPSP @ @ ;

: DO >LOOP >LOOP BEGIN ( store the loop parameters ) ;
: LOOP LOOP> LOOP> 1+ 2DUP = -ROT >LOOP >LOOP UNTIL LOOP> DROP LOOP> DROP ;
: I LOOPSP @ 8 - @ ;
...
: HASHTEST
S" this" 2DUP TELL HASH U. CR
S" is a" 2DUP TELL HASH U. CR
S" this " 2DUP TELL HASH U. CR
S" this i" 2DUP TELL HASH U. CR
S" this is" 2DUP TELL  HASH U. CR
S" this is " 2DUP TELL  HASH U. CR
;
